...
func (a *CSharpAdapter) ParseFile(rootPath, relPath string) (*FileIR, []Symbol, []Relationship, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to parse csharp file: %w", err)
	}
	...
	traverse = func(node *sitter.Node) {
		if node.Type() == "using_directive" {
		...
		} else if node.Type() == "class_declaration" || node.Type() == "interface_declaration" || node.Type() == "struct_declaration" {
			...
			if nameNode != nil {
				...
				id := fmt.Sprintf("cs://%s/%s", pkgName, name)
